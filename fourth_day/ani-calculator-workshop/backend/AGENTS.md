...
- **Unexpected error** (any status not produced by the calculator itself,
  e.g. `413` for an oversized request body, or `500` for a genuine bug):
  `{ "error": "The request could not be processed." }`. The real error is
  logged server-side only — the response never includes a stack trace or
  internal details.
